/**
 * Import a draht session shared as a gist by the issue-analysis CI workflow
 ...
 *   /ir https://github.com/draht-dev/draht/issues/123
 *
 *   draht "/ir <gist-id>"
 */
...
function decodeExportedHtml(html: string): { header: SessionHeader; jsonl: string } {
	...
	if (!match) throw new Error("HTML does not contain embedded draht session data");

	let data: unknown;
	try {
		data = JSON.parse(Buffer.from(match[1], "base64").toString("utf8"));
	} catch {
		throw new Error("embedded draht session data is not valid JSON");
	}
	...
	if (!header || header.type !== "session" || typeof header.id !== "string" || typeof header.cwd !== "string") {
		throw new Error("embedded draht session data has no valid session header");
	}
	if (!Array.isArray(sessionData.entries)) {
		throw new Error("embedded draht session data has no entries array");
	}
...
function getCiWorkdirName(sourceCwd: string): string | undefined {
	...
	return /^draht-ci-[0-9a-f]{32}$/i.test(name) ? name : undefined;
}
