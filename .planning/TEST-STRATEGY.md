...
- **`scripts/verify.sh` has a stale exclusion**: a comment claims `ci`/`knowledge` have "empty test suite files that vitest treats as errors" and excludes them from local verification. Both packages were run directly and pass cleanly (4 and 7 tests respectively) — the comment is stale, and local `verify.sh` currently diverges from what CI actually runs.
