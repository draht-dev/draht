...
- OAuth login flows are not supported in browser environments. Use the `@draht/ai/oauth` entry point in Node.js.
...
npx @draht/ai list               # list available providers
```
...
The library provides login and token refresh functions via the `@draht/ai/oauth` entry point. Credential storage is the caller's responsibility.
...
```typescript
import { loginGitHubCopilot } from '@draht/ai/oauth';
...
```typescript
import { getModel, complete } from '@draht/ai';
import { getOAuthApiKey } from '@draht/ai/oauth';
