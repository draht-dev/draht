...
function compressRequestBodyZstd(bodyJson: string): Uint8Array<ArrayBuffer> | null {
	...
	try {
		...
		// Use the ArrayLike overload (not (buffer, offset, length)) so the result is
		// typed Uint8Array<ArrayBuffer>, matching the DOM BodyInit/BufferSource type
		// expected by fetch()'s RequestInit.body. The (buffer, offset, length) overload
		// would infer Uint8Array<ArrayBufferLike>, which fetch's body type rejects.
