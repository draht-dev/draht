...
function geminiCliProjectId(credential: OAuthCredential): string {
...
export const geminiCliOAuth: OAuthAuth = {
	 ...
	 * project id, so the credential is handed to callers as a JSON blob in `apiKey`.
	 */
	async toAuth(credential) {
		return { apiKey: JSON.stringify({ token: credential.access, projectId: geminiCliProjectId(credential) }) };
	},
};
