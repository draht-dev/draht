...
type OAuthFlowLoaders = {
	...
	opencodeGo: () => OAuthAuth | Promise<OAuthAuth>;
...
export const loadOpenCodeGoOAuth = async (): Promise<OAuthAuth> => {
	if (bundledLoaders) return bundledLoaders.opencodeGo();
	return ((await importOAuthModule("./opencode-go.ts")) as { opencodeGoOAuth: OAuthAuth }).opencodeGoOAuth;
};

export const loadGoogleAntigravityOAuth = async (): Promise<OAuthAuth> => {
	if (bundledLoaders) return bundledLoaders.googleAntigravity();
	return ((await importOAuthModule("./google-antigravity.ts")) as { antigravityOAuth: OAuthAuth }).antigravityOAuth;
};

export const loadGoogleGeminiCliOAuth = async (): Promise<OAuthAuth> => {
	if (bundledLoaders) return bundledLoaders.googleGeminiCli();
	return ((await importOAuthModule("./google-gemini-cli.ts")) as { geminiCliOAuth: OAuthAuth }).geminiCliOAuth;
};
