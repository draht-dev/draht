...
describe("GitHub Copilot OAuth device flow", () => {
	...
	it("stops policy updates and persists authentication when the retry delay exceeds the login budget", async () => {
		...
		// Resolve the lazy OAuth module import before the fake-timer-driven login so the device poll
		// runs inside advanceTimersByTimeAsync instead of after it.
		await provider.auth.oauth?.toAuth({ type: "oauth", access: testCopilotAccessToken, refresh: "", expires: 0 });
