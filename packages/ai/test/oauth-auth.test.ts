...
describe.sequential("OAuthAuth adapters", () => {
	...
	// draht routes xAI OAuth requests through the Grok CLI proxy (SuperGrok subscription quota)
	// rather than the pay-per-token api.x.ai endpoint, so toAuth also pins the baseUrl.
	it("xAI toAuth derives the api key from the access token and pins the Grok CLI proxy baseUrl", async () => {
		const auth = await xaiOAuth.toAuth({ type: "oauth", access: "token", refresh: "r", expires: 0 });
		expect(auth).toEqual({ apiKey: "token", baseUrl: XAI_OAUTH_BASE_URL });
	});
