...
Extensions can register either a complete `@draht/ai` `Provider` or use the legacy provider-config form. Prefer a complete provider when custom authentication, filtering, refresh, or streaming behavior is required. draht composes `models.json` overrides above registered native providers.
...
The extension factory can also be `async`. For dynamic model discovery, fetch and register models in the factory instead of `session_start`. draht waits for the factory before startup continues, so the provider is available during interactive startup and to `draht --list-models`.
...
models: [{
  ...
  // ...
  reasoning: true,
  thinkingLevelMap: {              // map draht levels to provider values; null hides unsupported levels
    minimal: null,
    low: null,
    medium: null,
    high: "default",
...
Credentials are persisted in `~/.draht/agent/auth.json`:
...
- `stopReason === "error"`
- `errorMessage` matches one of draht's known overflow patterns (see [`packages/ai/src/utils/overflow.ts`](https://github.com/draht-dev/draht/blob/main/packages/ai/src/utils/overflow.ts))

If your provider returns overflow errors with a message draht does not recognize, normalize the error from the same extension that registers the provider. Use a `message_end` handler to rewrite the assistant message so its `errorMessage` starts with a phrase draht recognizes. The generic fallback `context_length_exceeded` is the safest choice.
...
interface ProviderModelConfig {
  ...
  /** Maps draht thinking levels to provider/model-specific values; null marks a level unsupported. */
