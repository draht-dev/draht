...
```bash
draht --mode json "Your prompt"
```

Outputs all session events as JSON lines to stdout. Useful for integrating draht into other tools or custom UIs.
...
```bash
draht --mode json "List files" 2>/dev/null | jq -c 'select(.type == "message_end")'
```
