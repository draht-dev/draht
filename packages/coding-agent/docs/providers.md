...
draht supports subscription-based providers via OAuth and API key providers via environment variables or auth file. Built-in catalogs ship with draht; configured providers may refresh newer catalogs and cache them in `~/.draht/agent/models-store.json` for offline use.
...
Use `/logout` to clear credentials. Tokens are stored in `~/.draht/agent/auth.json` and auto-refresh when expired. OpenRouter instead mints a user-controlled API key that does not expire automatically.
...
Store credentials in `~/.draht/agent/auth.json`:
...
```bash
draht --provider amazon-bedrock --model us.anthropic.claude-sonnet-4-20250514-v1:0
```
...
draht --provider amazon-bedrock --model arn:aws:bedrock:us-east-1:123456789012:application-inference-profile/abc123
```
...
draht --provider cloudflare-ai-gateway --model "claude-sonnet-4-5"
```
...
For normal draht usage, prefer unified billing or stored BYOK. Inline BYOK requires configuring an additional upstream `Authorization` header for the Cloudflare AI Gateway provider, for example via a `models.json` provider/model override.
...
export CLOUDFLARE_API_KEY=...           # or use /login
export CLOUDFLARE_ACCOUNT_ID=...
draht --provider cloudflare-workers-ai --model "@cf/moonshotai/kimi-k2.6"
```

draht automatically sets `x-session-affinity` for [prefix caching](https://developers.cloudflare.com/workers-ai/features/prompt-caching/) discounts.
...
draht supports the llama.cpp router server. Configure it with `/login llama.cpp`, manage loaded models with `/llama`, and select a loaded model with `/model`.
