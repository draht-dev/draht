> draht can create themes. Ask it to build one for your setup.
...
draht loads themes from:

- Built-in: `dark`, `light`
- Global: `~/.draht/agent/themes/*.json`
- Project: `.draht/themes/*.json` (only after the project is trusted)
...
On first run, draht detects your terminal background and defaults to `dark` or `light`.
...
```bash
mkdir -p ~/.draht/agent/themes
vim ~/.draht/agent/themes/my-theme.json
```
...
**Hot reload:** When you edit the currently active custom theme file, draht reloads it automatically for immediate visual feedback.
...
draht uses 24-bit RGB colors. Most modern terminals support this (iTerm2, Kitty, WezTerm, Windows Terminal, VS Code). For older terminals with only 256-color support, draht falls back to the nearest approximation.
