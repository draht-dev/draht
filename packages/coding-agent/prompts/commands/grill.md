---
description: "Grill any subject in whole-frontier question rounds — output form chosen at the end, not presumed"
---
...
`/brainstorm` remains the project-creation entry (hard gate → spec → `/new-project` or `/init-project`); `/grill` presumes nothing about where the conversation lands.
...
> **Tool note**: For environment facts, use the `subagent` tool with the `architect` agent — read-only fact-finding; never block the current round on it.
