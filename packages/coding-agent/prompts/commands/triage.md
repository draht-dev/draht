---
description: "Triage an external issue report — classify, bounded cause trace, GitHub Issues dedupe, fail-closed ticket creation, one typed verdict"
---
...
> **Tool note**: For the bounded cause trace, use the `subagent` tool with the `debugger` agent — findings-only diagnosis (no fixes, no writes); classification and every tracker write stay with the orchestrator.
