---
description: "Investigate why code is the way it is — parallel evidence investigators, confidence-tiered cited synthesis"
---
...
> **Tool note**: Use the `subagent` tool in **parallel mode** with the `investigator` agent — one task per available evidence category, dispatched in a single call. To escalate a contested synthesis, consult the `advisor` agent.
...
Dispatch one `investigator` agent per AVAILABLE category in one parallel-mode call; follow the subagent tool limits (max 8 tasks, 4 concurrent) — if the category count ever exceeds them, fold overflow into the nearest category. Skip rules:
