...
describe("AgentSession dynamic tool registration", () => {
	...
	it("exposes session state before custom bash spawn hooks and supports opting out", async () => {
		...
		expect(sessionEnv).toMatchObject({
			DRAHT_SESSION_ID: session.sessionId,
			DRAHT_SESSION_FILE: session.sessionFile,
			DRAHT_PROVIDER: model.provider,
			DRAHT_MODEL: model.id,
			DRAHT_REASONING_LEVEL: session.thinkingLevel,
		});
		...
		expect(optedOutEnv).not.toHaveProperty("DRAHT_SESSION_ID");
		expect(optedOutEnv).not.toHaveProperty("DRAHT_SESSION_FILE");
		expect(optedOutEnv).not.toHaveProperty("DRAHT_PROVIDER");
		expect(optedOutEnv).not.toHaveProperty("DRAHT_MODEL");
		expect(optedOutEnv).not.toHaveProperty("DRAHT_REASONING_LEVEL");
