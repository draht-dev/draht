...
describe("extensions discovery", () => {
	...
	it("loads the coding-agent entrypoint without rewriting pi-ai provider subpaths", async () => {
		fs.writeFileSync(
			...
			`
				import { getAgentDir } from "@draht/coding-agent";
				void getAgentDir;
				export default function(pi) {
					pi.registerCommand("test", { handler: async () => {} });
				}
	...
	it("keeps the type-only pi-ai OAuth compatibility barrel resolvable", async () => {
		fs.writeFileSync(
			...
			`
				import * as oauth from "@draht/ai/oauth";
				void oauth;
				export default function(pi) {
					pi.registerCommand("test", { handler: async () => {} });
				}
	...
	it("discovers subdirectory with package.json draht field", async () => {
	...
	it("keeps package.json draht extension entries with leading tilde package-relative", async () => {
	...
	it("package.json with draht field takes precedence over index.ts", async () => {
		const subdir = path.join(extensionsDir, "my-package");
		fs.mkdirSync(subdir);
	...
	it("ignores package.json without draht field, falls back to index.ts", async () => {
		const subdir = path.join(extensionsDir, "my-package");
		fs.mkdirSync(subdir);
		fs.writeFileSync(path.join(subdir, "index.ts"), extensionCode);
		fs.writeFileSync(
	...
	it("handles mixed direct files and subdirectories", async () => {
		...
		fs.writeFileSync(path.join(subdir2, "package.json"), JSON.stringify({ draht: { extensions: ["./entry.ts"] } }));
