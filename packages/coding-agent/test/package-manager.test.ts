...
describe("DefaultPackageManager", () => {
	...
	beforeEach(() => {
		previousOfflineEnv = process.env.DRAHT_OFFLINE;
	...
	afterEach(() => {
		if (previousOfflineEnv === undefined) {
			...
			process.env.DRAHT_OFFLINE = previousOfflineEnv;
		}
		vi.restoreAllMocks();
		vi.unstubAllGlobals();
...
Content`,
		...
		it("should auto-discover root markdown skills from Draht skill dirs", async () => {
...
Content`,
				...
				symlinkSync(sharedExtensionsDir, join(tempDir, ".draht", "extensions"), "dir");
				symlinkSync(sharedSkillsDir, join(tempDir, ".draht", "skills"), "dir");
				symlinkSync(sharedPromptsDir, join(tempDir, ".draht", "prompts"), "dir");
				symlinkSync(sharedThemesDir, join(tempDir, ".draht", "themes"), "dir");
	...
	describe("auto-discovered skill metadata", () => {
		it("should use the agent dir as baseDir for user .draht/agent skills", async () => {
		...
		it("should use the project .draht dir as baseDir for project Draht skills", async () => {
			const projectBaseDir = join(tempDir, ".draht");
	...
	describe(".agents/skills auto-discovery", () => {
		...
		it("should keep ~/.agents/skills user-scoped when cwd is under home in a non-git directory", async () => {
			...
			try {
				...
				const localAgentDir = join(tempDir, ".draht", "agent");
		...
		it("should dedupe user skill entries when ~/.draht/agent/skills is a symlink to ~/.agents/skills", async () => {
			const previousHome = process.env.HOME;
			process.env.HOME = tempDir;
	...
	describe("ignore files", () => {
		...
		it("should not apply parent .gitignore to .draht auto-discovery", async () => {
			writeFileSync(join(tempDir, ".gitignore"), ".draht\n");
	...
	describe("command spawning", () => {
		it("should preserve argv entries containing spaces", () => {
			...
			const valueWithSpace = "C:\\Users\\A B\\.draht\\npm";
	...
	describe("npmCommand", () => {
		...
		it("should update git package dependencies with --omit=dev", async () => {
			const source = "git:github.com/user/repo";
			const targetDir = join(tempDir, ".draht", "git", "github.com", "user", "repo");
			mkdirSync(targetDir, { recursive: true });
			writeFileSync(join(targetDir, "package.json"), JSON.stringify({ name: "repo", version: "1.0.0" }));
		...
		it("should install user npm packages into the Draht-managed npm root", async () => {
			settingsManager = SettingsManager.inMemory({
				npmCommand: ["pnpm"],
	...
	describe("settings source normalization", () => {
		...
		it("should store project local packages relative to .draht settings base", () => {
	...
	describe("pattern filtering in package filters", () => {
		...
		it("should resolve autoload-disabled package entries as positive-only without a global package", async () => {
			...
			settingsManager.setProjectPackages([
				{ source: relative(join(tempDir, ".draht"), pkgDir), autoload: false, extensions: ["+extensions/foo.ts"] },
			]);
...
export default function(api) { api.registerTool({ name: "test", description: "test", execute: async () => helper() }); }`,
	...
	describe("offline mode and network timeouts", () => {
		it("should update npm range packages using the configured spec", async () => {
			const installedPath = join(tempDir, ".draht", "npm", "node_modules", "example");
			...
			expect(runCommandSpy).toHaveBeenCalledWith(
				"npm",
				["install", "example@^1.0.0", "--prefix", join(tempDir, ".draht", "npm"), "--legacy-peer-deps"],
				undefined,
			);
		});
		...
		it("should batch npm updates per scope and run git updates in parallel while skipping pinned npm and current packages", async () => {
			...
			const projectOldPath = join(tempDir, ".draht", "npm", "node_modules", "project-old");
			const projectCurrentPath = join(tempDir, ".draht", "npm", "node_modules", "project-current");
		...
		it("should skip pinned packages when checking for updates", async () => {
			const installedNpmPath = join(tempDir, ".draht", "npm", "node_modules", "example");
