...
4. **Delegate to subagents in parallel via the Task tool** (dispatch all in the same assistant turn):
   ...
   - **(If spec context exists)** Task tool with `subagent_type: "spec-reviewer"` and prompt:
     ```
