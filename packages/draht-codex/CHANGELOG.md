...
- the `review` command template's step 7 now escalates a Critical finding whose refutation hinges on library or runtime behavior to a rung-4 script the orchestrator writes and runs against the real code (the review subagents cannot run commands) instead of settling it by argument — per the `blast-radius` skill
