...
function isCodexPluginInstalled(pluginSpec) {
	log("Checking whether the plugin is currently installed...");
	log("  $ codex plugin list --json");
	const result = spawnSync("codex", ["plugin", "list", "--json"], { encoding: "utf8" });
	if (result.status !== 0) throw new Error(`codex plugin list failed with exit code ${result.status}`);
	let output;
	try {
