...
4. **Spawn** — per selected beat: acquire the lease, then drive the inner cycle on the branch through Codex subagents with `spawn_agent`, `/orchestrate` discipline throughout:
...
Run this command's session on the strongest available Codex reasoning tier and let workers execute on the executor tier — the spawner is pure steering, its tokens are the ones that decide where all the volume tokens go (`model-tiering`).
