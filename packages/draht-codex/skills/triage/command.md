...
> **Tool note**: For the bounded cause trace, spawn a Codex subagent using the `debugger` agent prompt — findings-only diagnosis (no fixes, no writes); classification and every tracker write stay with the orchestrator.
