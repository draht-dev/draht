...
describe("TUI debug logging", () => {
	it("writes redraw logs to the provided directory", async () => {
		const logDir = mkdtempSync(join(tmpdir(), "draht-tui-log-"));
		try {
...
describe("TUI bounded render output", () => {
	it("splits a large full render without changing its output", async () => {
	...
	it("splits large differential updates without a full redraw", async () => {
		const terminal = new BoundedWriteTerminal();
		const tui = new TUI(terminal);
		const component = new TestComponent();
		tui.addChild(component);
