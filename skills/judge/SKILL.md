...
If `judge` is not on PATH, the same binary ships with this plugin at `<PLUGIN_ROOT>/bin/judge` — run it by path, and tell the user the plugin installer can link it onto PATH for them.
