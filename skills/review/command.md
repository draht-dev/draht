...
4. **Delegate to subagents in parallel** (dispatch all in the same assistant turn):

   - **Subagent** — dispatch the `reviewer` role with prompt:
     ```
   ...
   - **Subagent** — dispatch the `security-auditor` role with prompt:
     ```
   ...
   - **(If spec context exists)** dispatch the `spec-reviewer` role with prompt:
     ```
