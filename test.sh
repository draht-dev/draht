...
touch "$test_root/.draht-test-owned" "$test_root/npm-userconfig" "$test_root/npm-globalconfig"
...
cleanup() {
	...
	case "$test_root" in
		"$temp_parent"/draht-test.*)
			if [[ -d "$test_root" && ! -L "$test_root" && -f "$test_root/.draht-test-owned" ]]; then
				rm -rf -- "$test_root"
			else
